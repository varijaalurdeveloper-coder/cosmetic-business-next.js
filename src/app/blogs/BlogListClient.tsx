...
export default function BlogListClient() {
  ...
  const [posts, setPosts] = useState<BlogPost[]>(allBlogPostsSorted);
  ...
  const categoryTabs = useMemo(() => ["All Posts", ...categories], [categories]);
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-[54px] rounded-full px-4 text-center text-base font-medium transition-all sm:text-lg ${
                        isActive ? "bg-[#08c12d] text-white shadow-sm" : "bg-transparent text-[#111827] hover:text-green-700"
                      }`}
                    >
                      {category}
                    </button>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 px-1">
            <p className="text-sm text-[#4b5563] sm:text-base">
              {filteredPosts.length} articles{activeCategory !== "All Posts" ? ` in ${activeCategory}` : ""} • {categories.length} categories
            </p>
          </div>
          ...
          {filteredPosts.length > 0 ? (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredPosts.map((post) => (
                <Card key={post.id} className="overflow-hidden rounded-[24px] border-0 bg-white shadow-[0_8px_24px_rgba(16,24,40,0.08)] transition-transform duration-200 hover:-translate-y-1">
                  ...
                  <CardContent className="flex h-full flex-col p-6">
                    ...
                    <p className="mb-5 line-clamp-3 text-sm leading-6 text-[#6b7280]">{post.excerpt}</p>
                    ...
                    <div className="mt-auto border-t border-[#e5e7eb] pt-4">
                      <Link href={`/blogs/${post.slug}`} className="inline-flex items-center text-sm font-semibold text-[#08a526] transition-colors hover:text-green-700">
                        Read Article →
                      </Link>
                    </div>
                  </CardContent>
            ...
            <div className="rounded-[24px] bg-white px-6 py-14 text-center shadow-[0_8px_24px_rgba(16,24,40,0.08)]">
              <p className="text-base text-[#4b5563]">No blog posts found for your search.</p>
            </div>
          )}
        </div>
      </section>
